...
{
    /// <summary>
    /// An in-memory snapshot of the whole static reference-data graph — the 12 exported sets — that the
    ...
    /// caches (the admin "Content Health" view), so the same checker powers both (spike #1390, decision 5).
    /// </summary>
    public sealed record ContentGraph(
        IReadOnlyList<Contracts.Skill> Skills,
        IReadOnlyList<Contracts.Tag> Tags,
