...
{
    /// <summary>
    /// The shared proficiency-XP accrual step for a won battle (effect-based model, spike #1318,
    /// max-normalized per spike #1526 Decision 5). Given the battle's skill stats and the combatants' combat
    /// ratings, it sums each path's activity for its <see cref="EActivityKey"/>, routes it to the path's current
    /// frontier tier, and has each path independently claim <c>pie × activity ÷ max(playerRating, enemyRating)</c>
    /// of XP — max-normalization is the continuous difficulty curve (above your weight the claim rate is what a
    /// matched player would earn; at or below it, the treadmill is unchanged). It then applies the leveling
    /// against each proficiency's authored curve and writes the absolute result through the progress aggregate.
    /// Both the live battle-completion handler and the offline-rewards batch run it, so the accrual is computed
    /// identically on both paths (the "offline == live" invariant).
    /// <para>
    ...
    /// (<see cref="BattleStats.SunderBonusDealt"/> — #1429), the execute Cull share
    /// (<see cref="BattleStats.CullBonusDealt"/> — #1430), and the cadence Frequency share
    /// (<see cref="BattleStats.CadenceBonusDealt"/> — #1527, a pseudo-overlay riding every landed hit), each booked
    /// as the hit's health-capped damage × φ(the overlay's own investment) (#1481) — are damage-type-neutral
    ...
    public class ProficiencyRewardService(IProficiencies proficiencies, ISkills skills)
    {
            ...
            PlayerProgress progress, BattleStats stats, double ratingDenominator, Player player, bool notify)
        {
            var slices = ProficiencyXpCalculator.Split(
                ServerGameConstants.ProficiencyXpPerVictory, ratingDenominator, BuildActivities(stats, progress));
            if (slices.Count == 0)
            {
        ...
        //     (#1430), and the cadence pseudo-overlay Frequency (#1527, φ on effective cadence − 1, riding every
        //     landed hit) — plus dodged damage, healing done, reflected damage dealt, and counter damage dealt
        //     (#1457): damage-type-neutral magnitudes that map straight to a single activity key
        //     (Crit / Dodge / Heal / Reflect / Hex / Momentum / Sunder / Cull / Parry / Cadence) without applies() routing.
        private List<PathActivity> BuildActivities(BattleStats stats, PlayerProgress progress)
        {
            ...
            // Frequency (#1426/#1527) trains on the cadence pseudo-overlay share — landed damage × φ(cadence − 1).
