...
namespace Game.Application.Services
{
        ...
        ILogger<BattleService> logger)
    {
        ...
        {
            // Rate the player's rebuilt battler from the same frozen snapshot the battle was simulated against,
            // not the live aggregate. Valid mid-battle socket commands (stat reallocation, gear swaps) can shift
            // live power between battle start and the victory claim — which would both diverge from the fought
            // battle and let a client deflate its power right before claiming to inflate the payout. RecordVictory
            // only runs after TryResolveActiveBattle has confirmed an active snapshot, so a null here is a broken
            ...
            // The enemy is rated on its fielded loadout (BattleSkills, already selected by SimulateBattle before
            // this runs) — the measure prices the fight that actually happened (spike #1526 Decision 6).
            var enemyRating = CombatRating.Rate(enemy.ToBattler(), isPlayer: false);
            var rewards = new DefeatRewards(playerRating, enemyRating);

            // Snapshot the player's rating onto the battle stats so the proficiency accrual max-normalizes
            // activity by the identical measure the exp reward used.
            result.Stats.PlayerRating = rewards.PlayerRating;

            player.GrantExp(rewards.ExpReward);
            // Thread the player's rating onto the battle-completed event so the progress handler can
            // max-normalize each path's activity against it for the effect-based proficiency accrual (spike
            // #1526 Decision 5) — the same snapshot-measured rating the exp reward above used.
            player.RecordBattleCompleted(
                enemy, result, state.IsBossBattle, state.BattleZoneId ?? player.CurrentZoneId, timestamp,
                rewards.PlayerRating);
