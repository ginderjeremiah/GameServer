...
- **Two trigger kinds, both evaluated client-side** in `lib/engine/tutorials.ts` (there is no server push):
  - **Screen-anchored** — fires the first time its host screen becomes active, hooked at the single screen-activation point in `routes/game/+page.svelte` (screens are registry state, not routes), and plays immediately.
