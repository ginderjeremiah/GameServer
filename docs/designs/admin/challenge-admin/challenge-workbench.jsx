// ─────────────────────────────────────────────────────────────────────────
//  Challenge-aware Workbench — a faithful fork of Workbench2 that renders the
//  custom condition/reward sections via ChallengeSection and threads the live
//  sibling challenges into them (needed for reward exclusivity). Visually and
//  behaviourally identical to the other tools: list ←→ tabbed detail, diffing,
//  delete / restore / reset, validation triangles, save bar.
// ─────────────────────────────────────────────────────────────────────────
function ChWarnTri({ size = 13, title }) {
	return <span title={title} style={{ display: 'inline-flex' }}><window.Icon k="warn" size={size} sw={1.6} stroke="var(--warning)" /></span>;
}

// keys that constitute a "change" for each custom section (for the tab dirty dot)
const CH_CUSTOM_DIRTY = {
	'challenge-condition': ['type', 'statisticType', 'entityType', 'targetEntityId', 'progressGoal'],
	'challenge-reward': ['rewardItemId', 'rewardItemModId']
};

function ChallengeWorkbench({ entity, groupLabel }) {
	...
	const sectionDirty = (section, it) => {
		const b = baselineOf(it.id);
		if (!b) return false;
		if (section.kind === 'fields') return section.fields.some((f) => !window.eq(it[f.key], b[f.key]));
		const keys = CH_CUSTOM_DIRTY[section.kind];
		if (keys) return keys.some((k) => !window.eq(it[k], b[k]));
		return section.itemsKey ? !window.eq(it[section.itemsKey], b[section.itemsKey]) : false;
	};
					...
					<div style={{ padding: '16px 16px 12px', borderBottom: '1px solid var(--border-subtle)' }}>
						<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
							<div style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
								...
								<span className="meta">{liveItems.length}</span>
							</div>
								...
								<button key={it.id} onClick={() => setSelId(it.id)}
									...
									<div style={{ flex: 1, minWidth: 0 }}>
										<div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
											...
											{badge && <span className="rare-tag" style={{ color: 'var(--accent)', borderColor: 'currentColor' }}>{badge}</span>}
											{st === 'added' && <span className="spill added">new</span>}
											{st === 'modified' && <span className="spill modified">edited</span>}
											<div style={{ flex: 1 }} />
											{st !== 'deleted' && warns.length > 0 && <ChWarnTri title={warns.join(' · ')} />}
										</div>
										<span className="meta">
				...
				<div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
					<div style={{ padding: '18px 32px 0' }}>
						<div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
							...
							{entity.listBadge(sel) && <span className="rare-tag" style={{ color: 'var(--accent)', borderColor: 'currentColor' }}>{entity.listBadge(sel)}</span>}
						...
						{/* objective preview under the title */}
						<div className="ch-headline">{window.challengeSentence(sel)}</div>
						{/* tabs */}
						<div style={{ display: 'flex', gap: 4, marginTop: 14, borderBottom: '1px solid var(--border-subtle)', flexWrap: 'wrap' }}>
					...
					<div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
						<div style={{ maxWidth: 1020, opacity: selStatus === 'deleted' ? 0.5 : 1, pointerEvents: selStatus === 'deleted' ? 'none' : 'auto' }}>
							...
							<window.ChallengeSection section={curSection} item={sel} baseline={baseline} patch={patch} allItems={liveItems} />
						</div>
					</div>
...
window.ChallengeWorkbench = ChallengeWorkbench;
