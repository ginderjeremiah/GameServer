...
- **Tutorial blurb** — a short, in-context explanation. Use this for mechanics a player can grasp from **one good explanation** (what a crit is, how cooldowns charge). A blurb is a **lesson** (spike #1392): Workbench-authored reference data with a trigger of one of two kinds — **screen-anchored** (fires on first visit to a given screen; plays immediately) or **mechanic-anchored** (fires on a client-detected event like a first crit; queues as unread since the player may be AFK) — presented as an anchored coach-mark tour, not a toast. _(Live: the reference-data/authoring (#1591), tour component (#1592), per-player state (#1588), and trigger evaluation/unread indicator (#1587). The Help screen reading room — replaying a lesson from a full list — is the remaining follow-up, #1589.)_
...
- **Enemy power scaling:** every enemy is `BaseAmount + AmountPerLevel × level` per attribute (same shape as a class's locked base). Decide the **per-zone power target** and let level-band + distribution hit it. _(Record target power per zone band.)_
  - **Enemy cadence is authored directly, not via Agility ([#1426](./spikes/1426-attribute-identity-sourcing.md)).** With CDR severed from the core attributes, an `Agility` distribution no longer speeds an enemy up — a **fast enemy authors `CooldownRecovery`** (its base-1 charge multiplier) **or the `CooldownBonus` enabler** directly in its `AttributeDistribution`. Enemy distributions have **no core-only validation**, so authoring a derived/secondary attribute is allowed and is the intended lever. The combat rating ([#1531](https://github.com/ginderjeremiah/GameServer/issues/1531)) taxes an enemy's `AGI`/`LUK`/`DEX` points that nothing in its kit consumes down to zero contribution (rather than inflating its pricing, as the old core-attribute sum did) — keep the fingerprint matched to the kit anyway, since a dead-stat distribution is still wasted authoring intent (the content lint in [#1529](https://github.com/ginderjeremiah/GameServer/issues/1529) will flag it).
- **Difficulty & reward:** XP/kill is an enemy-authored bounty, `k × EnemyRating × min(EnemyRating ÷ PlayerRating, 1)²` — no upward premium for punching up, quadratically less for a trivial enemy ([details](./game-design.md#experience-rewards)). Both ratings come from the combat rating, not a raw attribute sum. Tune zone power so the intended enemy's rating sits near the player's for a player progressing normally.
- **Proficiency pacing:** per-tier curve is `BaseXp × XpGrowth^level` with a per-tier `MaxLevel`; per-victory magnitude is `ServerGameConstants.ProficiencyXpPerVictory`, normalized by `max(PlayerRating, EnemyRating)`. _(Record target tiers-per-zone so path progression tracks the zone arc.)_
...
- **The tally is a uniform share claim on the landed hit** (settled in the [spike](./spikes/1398-utility-in-proficiency-system.md#second-refinement--share-claims-supersede-the-counterfactual-marginal-1481), #1481 superseding the earlier counterfactual normalized-marginal shape). An **overlay** archetype (crit, Hex, Sunder, Momentum, Cull — ones that ride a hit that would have landed anyway) books, per hit it was active on, that hit's **landed** (health-capped) damage × `φ(investment)`, where `φ(a) = a/(1+a)` saturates the overlay's own investment. Strength-proportionality lives in `φ`, and because the landed basis sums to at most the enemy's health pool per won battle, stacked overlays pay out in **kill speed, not tally inflation** — no counterfactual baseline to run. This is applied to **crit too** ([#1448](https://github.com/ginderjeremiah/GameServer/issues/1448) is the worked reference, reshaped by #1481); Reflection/DoT are not overlays and keep their current tallies. _Tuning watch-point:_ a mitigation debuff's *value* is hyperbolic in enemy resistance, so author enabler strengths so a debuff can't erase a resist-gated boss.
...
- **Tutorials — mostly landed.** The arc leans on **tutorial blurbs** as a teaching lane (§2); the feature is scoped in spike **[#1392](https://github.com/ginderjeremiah/GameServer/issues/1392)** (lessons are Workbench-authored reference data, presented as an anchored coach-mark tour). The reference-data entity, Workbench authoring, per-player state, tour component, and client-side trigger evaluation/unread indicator are all live (#1591/#1588/#1592/#1587); only the Help screen reading room (browsing/replaying the full lesson list, #1589) remains.
