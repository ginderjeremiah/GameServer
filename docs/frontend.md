...
- **Not everything is a Workbench entity — Ops tools live alongside it.** Operations/diagnostics surfaces (the player write-behind dead-letter queue, `admin/ops`) are a different *kind* of tool: no catalogue, no save bar, just inspect/act over a live backend resource. They get their own sidebar group (`nav.ts` appends the **Ops** group + tools to the Workbench-derived nav) and `+page.svelte` branches on the active tool key to render their own view instead of `Workbench.svelte`. Such a view owns its state in a reactive controller class (`*.svelte.ts`), reads/writes over HTTP via `ApiRequest`, and guards mutating actions behind `confirmModal`. When adding an Ops tool, follow this split rather than bending it into an `EntityConfig`.
