...
- **`src`** — Svelte components, stores, and app code.
  - **`components`** — shared components, including reusable building blocks worth composing over re-implementing: the collapsible side rails (`components/sidebar`), the tooltip chrome (`components/tooltip` — `TooltipShell` plus section/title/stats primitives and their sealed/teaser counterparts), and `AttributeTooltip` (drive it through `createAttributeTooltip()`; publish the controller via context for descendant hover triggers). Extend these primitives rather than copying them.
