...
- **All colour flows from the root vars.** Every neutral/alpha shade is `color-mix(in srgb, var(--…) N%, transparent)` over a base token (`--accent`, `--text-primary`, `--error`, `--success`) or an existing semantic var (`--text-muted`), and the dark-on-accent button text uses `--text-on-accent` — no hard-coded `rgba(…)`/hex, so a theme can restyle the screen by overriding the variables alone.
