...
- **Live preview reuses the real log row.** The right-hand preview renders a self-contained sample event stream through the actual `LogRow` component, filtered by the _draft_ preferences — so it looks exactly like the real combat log and visibly reflects each toggle before you save. The semantic log palette is the single source of truth (`logColors`, exported from `log-kind.ts`) shared by the real log, the preview and the per-type rows.
