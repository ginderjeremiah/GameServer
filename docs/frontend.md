...
- **Dismissal can trigger a follow-up action.** An optional `onDismiss` callback runs once a toast is dismissed — by the manual control or by auto-dismiss, but **not** by a bulk `clearToasts` reset. This drives the `SocketReplaced` flow (`$lib/engine/engine.ts`): when the backend reports the player's single allowed connection was taken over by another session, the engines are stopped and a sticky warning toast is shown; dismissing it returns the player to login via a client-side `goto('/')` — deliberately not a full page reload, which would re-run the boot gate and bounce a still-authenticated client back through the loading screen.
