...
- **Challenges are type-derived, not free-form.** The Challenges tool (`entities/challenge.ts`, custom `challenge-condition` / `challenge-reward` sections) treats the `ChallengeType` (from `GET /api/Challenges/ChallengeTypes`) as the single source of truth: a challenge's tracked **statistic**, target-**entity type**, and **goal comparison** are all derived from the chosen objective type and shown read-only — picking the type re-derives them (`deriveFromType` in `entities/challenge-helpers.ts`), so they can never be set inconsistently. The whole challenge is its own primary DTO (no child collections), and the server re-derives statistic/entity on save, so `persist` posts a plain `AddEditChallenges` batch with no child savers. Rewards enforce **hard exclusivity** — an item/mod already unlocked by another challenge is disabled in the picker — so a reward can't be double-assigned. The amber dot/triangle split is unchanged: dot = unsaved, triangle = a validation warning (here a non-positive goal, a missing name/description, or a reward-less challenge).

## Engine singletons & import cycles

The game engine (`src/lib/engine`) exposes app-wide singletons (`playerManager`, `inventoryManager`, `enemyManager`, plus the logic/render/battle engines). Each manager singleton is instantiated **in the same module that defines its class** (e.g. `playerManager` lives at the bottom of `player/player-manager.ts`), not centralized in `engine.ts`.

This is deliberate: `log.ts` reads `playerManager.logPreferences` to gate messages, and `PlayerManager` calls `logMessage`, so the two modules form a (benign, call-time-only) cycle. When the singleton was created in `engine.ts`, importing `log` pulled the barrel → `engine.ts`, which ran `new PlayerManager()` while `player-manager.ts` was still mid-evaluation, throwing `PlayerManager is not a constructor` for any entry point other than the barrel itself (e.g. a unit test importing `player-manager.ts` directly). Co-locating the singleton with its class means the cycle's back-edge (`log` → `playerManager`) targets the already-evaluating leaf module and never triggers eager instantiation from `engine.ts`. Internal engine modules should therefore import a manager singleton from its defining module, and unit tests should `vi.mock` that same path. Avoid reintroducing `new PlayerManager()` (or the other managers) into `engine.ts`.
