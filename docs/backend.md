...
- **`.claude/hooks/session-start.sh`** runs at Claude Code session start. When Docker is fully functional (bridge networking present) it is a **no-op** and removes any stale marker, so local dev and CI keep using Testcontainers. Otherwise it starts a sandbox-compatible `dockerd` (`--storage-driver=vfs --iptables=false --ip6tables=false`), launches PostgreSQL and Redis with **host networking** on fixed ports (5499 / 6399, using the same images as the fixtures), and writes a `.container-info.json` marker at the repo root. It is best-effort and never fails the session.
...
**Isolation assumption:** the reuse path shares one PostgreSQL database and one Redis instance across all integration tests. This is safe because every integration test runs in the single `Integration` xUnit collection (serialized within an assembly) and `ApplicationIntegrationTestBase.CleanupAsync` truncates player/game data and flushes Redis before each test. It also assumes the integration-test **assemblies** run sequentially (the `dotnet test` default); running them concurrently against the shared services would interfere. Normal (Testcontainers) runs are unaffected because each fixture gets its own isolated container.
