...
Layered on top of the per-IP limiter as defence-in-depth against a **slow, distributed** credential guess on one account that stays under any single IP's rate limit, `Login` applies a **per-account exponential backoff**. Consecutive failures are tracked in Redis keyed by a hash of the (case-sensitive) username and **reset on a successful credential verification**; the first few failures (a configurable threshold) are free, after which each further failure delays the next permitted attempt by a doubling window up to a **low cap**. It is deliberately a bounded **backoff, never a hard lockout** — a legitimate user is delayed by at most the cap — so an attacker who knows a username can only briefly slow the owner, not lock them out. A backed-off attempt is rejected **before** any database hit or PBKDF2 work (so spamming during an active window neither extends it nor costs server CPU) and surfaces as a 429 with a `Retry-After` header and a message distinct from invalid-credentials. The keying is per-account, not per-(account × IP): per-(account × IP) would let each distributed bot keep its own counter and add little over the IP limiter. Like the IP limiter the parameters are config-bound (`LoginBackoff` section) with safe defaults and validated as sane on start. The backoff arithmetic is a pure, unit-tested policy; the Redis read-modify-write and the system clock (an injected `TimeProvider`) sit in a thin guard around it.
