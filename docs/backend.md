...
On the `ICacheService` cache contract, the value-accepting setters fall into two camps. `Set` and `SetAndForget` (both their plain and expiry overloads) take a **nullable** value where a **null value deletes the key** — the de-facto null-means-delete behaviour their generic `<T>` overloads rely on. The `GetSet` overloads are **non-null**: the atomic set-with-expiry `GetSet(key, value, expiry)` because writing a TTL implies writing a value (a null would error its server-side Lua `SET`), and the non-expiry `GetSet(key, value)` because the underlying single-round-trip `GETSET` rejects a null value outright rather than deleting — so it has no delete path to expose.
