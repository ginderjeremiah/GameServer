...
7. **Replace all proficiency seeds (gateways retained on implementation).** The only nodes that genuinely needed a seed were cross-path gateways (a node with no contributing skills); within-path tiers are already trainable by existing path skills at falloff. So: delete `SeedSkillId`; a within-path tier's full-pace native comes from the **previous tier's mastery milestone reward** (default) or an optional **upgrade recipe** (the "fuse two techniques into a stronger one" case); a synthesized line bootstraps its native through a **recipe's** input skills + proficiency conditions. Every path then opens one uniform way — a contributing skill accrues XP. _Implementation refinement (area C / #1297):_ only the **seed** layer was removed; the `ProficiencyPrerequisite` cross-path **gateway** system was **retained**, earning a renewed role under the proficiency-progression-avenues rework ([#1318](https://github.com/ginderjeremiah/GameServer/issues/1318)).
...
### C. Proficiency simplification: remove seeds ([#1297](https://github.com/ginderjeremiah/GameServer/issues/1297))
Delete `SeedSkillId` (entity/core/contract/mapper, the admin seed picker, the seed-grant in `ProficiencyRewardService.Open()`) and drop the column. Opening a tier (in-path successor or satisfied gateway) stays as **notification-only** — no skill is granted; re-home a within-path tier's native onto the predecessor tier's max-level milestone reward. **Scope note (landed):** the **cross-path gateway** machinery (`ProficiencyPrerequisite`, `ProficiencyPrerequisiteGraph`, `DependentsOf`, the gateway branch of `OpenSuccessors`/`AllPrerequisitesMaxed`, and gateway authoring) was **retained** — it earns a renewed purpose under the proficiency-progression-avenues rework ([#1318](https://github.com/ginderjeremiah/GameServer/issues/1318)), so only the seed layer was removed here. Depends on A, B.
...
| [#1297](https://github.com/ginderjeremiah/GameServer/issues/1297) — Remove proficiency seed skills (cross-path gateways retained for [#1318](https://github.com/ginderjeremiah/GameServer/issues/1318)) | C | #1295, #1296 | medium |
