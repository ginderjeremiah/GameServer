...
- **[#552](https://github.com/ginderjeremiah/GameServer/issues/552)** _(tech debt, claude, scope: small)_ — Trim the write-behind publish: `FireAndForget` the wake-publish in `RedisPubSubService.Publish(channel, queue, data)`. The wake was already at-most-once (the awaited LPUSH is the durable part), so this drops a round-trip from every save. The batch-LPUSH part was split to **[#559](https://github.com/ginderjeremiah/GameServer/issues/559)** (off the hot path — a battle-end raises ~1 player event + 1 separate progress event — and it needs a buffering refactor). Verifying the wake caveat surfaced **[#560](https://github.com/ginderjeremiah/GameServer/issues/560)** (drain the queue on synchronizer startup). Independent of #550.

Measured effect of #550 + #552 on the battle-end response path (dev box, median): full round trip **4.91 ms → 2.19 ms**; progress read **2.17 ms cold → 0.81 ms cache hit**; `SavePlayer` publish **1.19 ms → 0.82 ms** (FireAndForget); `Commit` **1.59 ms → 0.27 ms** (write moved off-path).
