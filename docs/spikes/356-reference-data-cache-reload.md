...
- ✅ **[#359](https://github.com/ginderjeremiah/GameServer/issues/359)** _(enhancement, claude)_ — Cross-instance invalidation: publish a reference-data-changed message on admin writes and background-reload on every instance, with burst coalescing. Depends on #358. **Done** — see [Reference-data cache reload](../backend.md#reference-data-cache-reload-build-then-swap).
