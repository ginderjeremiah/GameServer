...
- **Flat-mitigation (Defense & Block) rework** — *designed during this spike, intentionally not ticketed yet* (the project owner is reworking it conversationally with full context). Flat subtraction is scale-dependent (a fixed `Defense` is overpowering against small hits and worthless against big ones, with a degenerate immunity breakpoint at the `0` floor) — exactly the "useless or too powerful" feel to fix. The recommended end-state: convert the bulk stat into a **percentage via a diminishing curve** (`defReduction = Armor / (Armor + K·attackerLevel)`), applied as a multiplier instead of a subtraction. This gives **linear effective-HP** (each point of Armor is worth a constant % EHP — never useless) while the % asymptotes below 100% (never immune, no breakpoint), and `K` scaling with attacker level keeps it relevant at high zones. **Block** gets the same treatment: a successful block applies a **% reduction (`BlockPower`)** on its multiplicative chain rather than a flat `BlockReduction`. End-state mitigation becomes fully multiplicative — `dealt × (1 − Σ typeResist) × (1 − defReduction) × (blocked ? 1 − BlockPower : 1)` — at which point step 6's absorption branch disappears (nothing flat to floor). This is a **localized swap of the final mitigation step**: it does not touch the `applies()` map or the type-resist math, so it is safe to land before or after this spike. V1 keeps flat Defense/Block as-is.
