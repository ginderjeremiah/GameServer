...
**Area D — verified end-to-end** (#1326). Enemies are first-class typed participants with **no** battle-pipeline change: resistance/amplification are authored as ordinary `AttributeDistribution` rows (a damage-type attribute is just another `EAttribute`), and an enemy's fielded skills are typed automatically because `BattleSkill` routes `Skill.DamagePortions` through `DamageTarget` regardless of attacker. `TypedEnemyCombatTests` builds a real `Enemy` through the production resolve-then-build path and confirms a fire-resistant enemy takes reduced fire damage (physical untouched), a vulnerable one extra, an immune one none, an absorbing one heals, an `ElementalResistance` enemy resists fire through the shared category key, the resistance scales at the enemy's level, and an enemy fielding a fire skill deals resistible/amplifiable typed damage to the player. `AdminEnemiesIntegrationTests.SetAttributeDistributions_PersistsDamageTypeResistanceAndAmplification` pins the authoring round-trip. Example resistant/vulnerable/immune/absorbing enemies are realized **in-test** rather than committed, since static content lives in the DB/Workbench and is not source-controlled (the export/seed tooling is the separate spike #1390).
