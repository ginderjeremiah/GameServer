...
- **Adopt the rarity hue/label** on the surfaces that currently stub a neutral accent:
  - `challenge-unlocks.ts` — the shared resolver: skill reward uses `rarityColor(skill.rarityId)` / `rarityLabel(skill.rarityId)`, `sub: \`${rarityLabel} · Skill\``, and `rarity: skill.rarityId` (drops the `Common`/neutral accent fallback, so skill rewards sort and tint by real tier alongside items/mods).
  - `challenges-view.svelte.ts` — the challenges-screen view-model that layers the **glow** on top of the shared resolver. It still special-cases skills: the `ResolvedReward` rarity/accent/glow comments and, load-bearing, `glow: base.kind === 'skill' ? null : rarityGlow(base.rarity)`. Drop the `base.kind === 'skill' ? null :` guard so the glow becomes `rarityGlow(base.rarity)` uniformly — otherwise editing only `challenge-unlocks.ts` fixes the sort/accent but the glow stays suppressed. Update `challenges-view.svelte.test.ts` coverage.
