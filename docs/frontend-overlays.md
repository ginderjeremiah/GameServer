...
- **Two sides of the single-connection model.** The `SocketReplaced` handler is the *losing* session's experience; the *winning* side is warned **before** taking over — on login, `confirmSessionTakeover()` calls the authenticated `Auth/ActiveSession` endpoint (over HTTP, so it never opens a socket of its own) and confirms if the player already has a live connection elsewhere. It fails open (a non-200 proceeds rather than blocking a legitimate login) and runs only on explicit login, not the silent resume path.
...
- **Tab-trap, Escape, scroll-lock, and focus restore are the shared `focusTrap` action** (`components/focus-trap.ts`), used by both `ModalHost` and `Popover`; only the per-open initial focus is component-specific (kind-aware here, first-focusable in `Popover`). Reuse the action for any new overlay rather than re-implementing the trap. Traps stack: when overlays layer (e.g. a confirm modal opened from inside the trapped inventory item drawer), only the most recently mounted trap handles Tab/Escape, so the outer one can't steal focus or double-dismiss.
...
An overlay whose shell must stay mounted while closed (the inventory item drawer keeps its shell in the DOM so its slide transform can transition) can't compose `Popover`'s `{#if}`-mounted layer; it applies `focusTrap` to its `{#if}`-mounted *content* instead and mirrors `Popover`'s first-focusable capture, so the same Escape/trap/restore chrome holds.
