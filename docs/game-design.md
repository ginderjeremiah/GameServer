...
A zone is considered **cleared** when the player wins a battle against a boss enemy (`Enemy.IsBoss`) while idling in that zone. On a boss victory, `ZonesCleared` is incremented both globally and for the player's current zone (the per-zone entry is keyed by zone id), so a `ZonesCleared` challenge can target either "clear any zone" or a specific zone. This keeps the statistic distinct from the per-enemy `BattlesWon` and the global `BossesDefeated` counts, and requires no extra battle-event context — the zone is read from the player's `CurrentZoneId` at the time the battle resolves.

This is the meaning that fits the current design, where bosses are enemies flagged `IsBoss` that appear in a zone's random spawn table. The intended longer-term design — a single dedicated boss per zone, fought via a dedicated "Challenge Boss" action — is tracked in issue #96; once that lands, the clear condition narrows to "defeated this zone's dedicated boss".
