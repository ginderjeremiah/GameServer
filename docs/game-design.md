...
- **Stack, don't refresh.** Re-applying an already-active effect adds a *second* timed modifier rather than refreshing the first — the magnitudes sum (additive amounts add, multiplicative factors compound) and each application expires on its own schedule. So a 10 dmg/s poison re-applied while a 5s one is still active reads as 15 dmg/s until the first expires, then 10. Distinct effects on the same attribute stack the same way. Stacking is unbounded (a per-effect cap is a possible later balancing lever).
