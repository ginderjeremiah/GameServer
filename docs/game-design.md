...
- **No grandfathering (deliberate).** The unlock rule applies uniformly to everyone: an authored gate locks any player who has not completed its challenge, including existing players. There is no migration that backfills clears or treats a player's current high-water-mark zone as reached. A player keeps their current zone (locking never forcibly relocates them — it gates *navigation* only), but advancing past an authored gate requires completing it. This was chosen over a frontend/`currentZone` grandfather or a backend backfill to keep the rule a single, honest predicate with no special cases.
