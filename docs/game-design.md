...
- **Stack the magnitude, share one expiry.** Re-applying an effect adds another timed modifier so the magnitudes sum (additive amounts add, multiplicative factors compound), but every active application **on the same attribute** shares a **single expiry that resets to the newest application's duration** — there are *no* independent per-portion expirations. Following the canonical example: apply 10 DoT for 5s, then 2s later apply 20 DoT for 2s → the target now has 30 DoT for 2s (the older application is cut from its remaining 3s down to the new 2s); 1s later apply 10 DoT for 5s → 40 DoT for 5s (the whole stack adopts the new 5s). Distinct effects on the same attribute stack and share the expiry the same way; effects on **different** attributes remain independent. Stacking is unbounded (a per-effect cap is a possible later balancing lever).
