...
namespace Game.Core.Tests.Battle.Offline
{
    ...
    public class OfflineProgressResultTests
    {
        ...
        private static OfflineBattleOutcome Win(int enemyId, int totalMs, int exp) =>
            new(MakeEnemy(enemyId), new BattleResult(Victory: true, PlayerDied: false, totalMs, new BattleStats()), exp, PlayerRating: 100.0);

        private static OfflineBattleOutcome Loss(int enemyId, int totalMs) =>
            new(MakeEnemy(enemyId), new BattleResult(Victory: false, PlayerDied: true, totalMs, new BattleStats()), ExpReward: 0, PlayerRating: 0);

        private static OfflineBattleOutcome Draw(int enemyId, int totalMs) =>
            new(MakeEnemy(enemyId), new BattleResult(Victory: false, PlayerDied: false, totalMs, new BattleStats()), ExpReward: 0, PlayerRating: 0);
