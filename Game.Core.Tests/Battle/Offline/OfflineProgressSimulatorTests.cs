...
namespace Game.Core.Tests.Battle.Offline
{
    ...
    public class OfflineProgressSimulatorTests
    {
        ...
        public void Simulate_PerVictoryExp_MatchesDefeatRewardsFromSnapshot()
        {
            ...
            Assert.All(result.Battles, battle =>
            {
                var expected = new DefeatRewards(playerBattler, battle.Enemy).ExpReward;
        ...
        public void Simulate_SnapshotsPlayerRatingOntoEachVictoryStats()
        {
            ...
            var expectedRating = CombatRating.Rate(
                scenario.Snapshot.ToBattler(scenario.ResolveItem, scenario.ResolveMod, scenario.ResolveSkill), isPlayer: true);
        ...
        private static BattleSnapshot PlayerSnapshot(
            ...
            IEnumerable<EquippedItemSnapshot>? equippedItems = null)
        {
            return new()
            {
                Level = 1,
                ...
                EquippedItems = equippedItems?.ToList() ?? [],
                SkillIds = [0],
            };
        }
