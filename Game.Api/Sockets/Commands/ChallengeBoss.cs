...
namespace Game.Api.Sockets.Commands
{
    ...
    /// same anchored-cooldown handshake — whenever a post-battle cooldown is in effect (this call's own
    /// abandon resolved a win/loss/draw, or one was already running from an earlier battle, #1920), the
    /// returned battle's start is anchored to that cooldown's expiry and <see cref="NewEnemyModel.Cooldown"/>
    /// tells the client to wait it out before treating the boss as live.
    ...
    public class ChallengeBoss : AbstractSocketCommand<NewEnemyModel, ChallengeBossRequest>
    {
        ...
        public override async Task<ApiSocketResponse<NewEnemyModel>> HandleExecuteAsync(SocketContext context, CancellationToken cancellationToken)
        {
            ...
            // Still in the future whenever StartBossBattle anchored the returned battle to an in-effect
            // cooldown rather than now (#1920) — the client must wait it out before treating the boss as
            // live, mirroring NewEnemy's identical handshake.
