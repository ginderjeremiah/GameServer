...
namespace Game.Api.Tests.CodeGen
{
    public class SocketMapWriterTests : IDisposable
    {
        ...
        [Fact]
        public void WriteSocketMap_CommandsOrderedOrdinally_NotByHostCulture()
        {
            // The real GetChallengeTypes/GetChallenges pair: ordinally 'T' precedes 'e', while
            // culture-aware collation compares case-insensitively first and orders them the other way,
            // so the byte-compared output would depend on the generating machine's locale.
            var writer = new SocketMapWriter(_options);
            var metadata = new List<SocketCommandMetadata>
            {
                new(typeof(TestSocketCommandBasic)) { CommandName = "alpha" },
                new(typeof(TestSocketCommandBasic)) { CommandName = "GetChallenges" },
                new(typeof(TestSocketCommandBasic)) { CommandName = "GetChallengeTypes" },
            };

            writer.WriteSocketMap(metadata, "// Auto-generated");

            var content = File.ReadAllText(Path.Combine(_options.TargetDirectory, "api-socket-type-map.ts"));
            var typesIndex = content.IndexOf("'GetChallengeTypes'", StringComparison.Ordinal);
            var challengesIndex = content.IndexOf("'GetChallenges'", StringComparison.Ordinal);
            var lowercaseIndex = content.IndexOf("'alpha'", StringComparison.Ordinal);
            Assert.True(typesIndex >= 0 && typesIndex < challengesIndex);
            Assert.True(challengesIndex < lowercaseIndex);
        }
