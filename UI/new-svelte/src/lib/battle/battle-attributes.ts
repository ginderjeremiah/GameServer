...
const calculateDerivedStats = (attributes: number[]) => {
	attributes[EAttribute.MaxHealth] +=
		50 + 20 * attributes[EAttribute.Endurance] + 5 * attributes[EAttribute.Strength];
	attributes[EAttribute.Defense] +=
		2 + attributes[EAttribute.Endurance] + 0.5 * attributes[EAttribute.Agility];
	attributes[EAttribute.CooldownRecovery] +=
		0.4 * attributes[EAttribute.Agility] + 0.1 * attributes[EAttribute.Dexterity];
...
export const newBattleAttributes = (
