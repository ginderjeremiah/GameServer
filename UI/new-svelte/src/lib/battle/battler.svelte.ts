...
export interface Battler {
	...
	isDead: boolean;
	reset(
		battlerData: { attributes: IBattlerAttribute[]; selectedSkills: number[]; level: number },
...
const fillSelectedSkills = (battlerData: BattlerData, battler: Battler) => {
	...
	const skills: (Skill | undefined)[] = battlerData.selectedSkills.map((skillId) =>
		newSkill(skillData[skillId], battler)
	);
