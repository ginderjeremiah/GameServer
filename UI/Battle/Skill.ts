...
export class Skill extends Tooltippable implements ISkill {
  ...
  updateTooltipData(
    ...
    let remainingCd =
      (this.cooldownMs - this.chargeTime) /
      ((1 + this.owner.attributes.getValue(EAttribute.CooldownRecovery) / 100) *
        1000);
  ...
  calculateDamage() {
    ...
    this.damageMultipliers.forEach((dmgType) => {
      dmg +=
        this.owner.attributes.getValue(dmgType.attributeId) *
        dmgType.multiplier;
    });
    return dmg;
  }
  ...
  createDamageListContent(damageList: HTMLUListElement, target: Battler) {
    ...
    this.damageMultipliers.forEach((mult) => {
      ...
      let dam =
        this.owner.attributes.getValue(mult.attributeId) * mult.multiplier;
      multiplier.textContent =
        attributeData[mult.attributeId].name +
        ": " +
        formatNum(dam) +
        " (" +
        formatNum(mult.multiplier) +
        "x)";
      totDam += dam;
    ...
    if (target) {
      ...
      let adjDps =
        (totDam - target.attributes.getValue(EAttribute.Defense)) / cd;
      adjDamagePerSecond.textContent =
        "Adjusted DPS: " + formatNum(adjDps) + "/s";
