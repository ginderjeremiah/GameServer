...
const { mockSkills, mockEnemies, mockAttributes, mockPlayerManager, mockInventoryManager, mockPlayerProficiencies } =
	vi.hoisted(() => {
		...
		const mockInventoryManager = {
			...
			// The parry counter skill id (#1457), threaded alongside the weapon type; undefined here (the
			// mock equips no weapon and seeds no punch), so the rebuilt battler carries no counter.
			counterSkillId: undefined as number | undefined
		};
