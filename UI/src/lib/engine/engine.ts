 ...
 * level-ups, milestones, and newly-opened proficiencies, makes any milestone skill usable immediately
 * (mirroring the challenge-reward unlock), then updates the proficiency store so the tree and the live
 * battler's bonuses reflect the gain without a refetch. Prior levels are read off the store before it is
 * updated, since the push carries only the new level — the level-up is detected by comparison.
 */
...
/** Surfaces a newly-opened proficiency (a maxed tier's next tier within its path). Both a combat-log line
 *  and a toast, since an unlock is a notable, infrequent event. Unknown reference ids are skipped. */
