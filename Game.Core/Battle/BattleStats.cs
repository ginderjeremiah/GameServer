namespace Game.Core.Battle
{
    public class BattleStats
    {
        ...
        /// <summary>
        /// The player's <see cref="CombatRating"/> for this battle, the same rating <see cref="DefeatRewards"/>
        /// uses for the bounty curve. The effect-based proficiency accrual max-normalizes each activity against
        /// <c>max(playerRating, enemyRating)</c> (spike #1526 Decision 5), so it must be the identical measure
        /// to avoid diverging from the reward that was actually paid. Populated at battle completion from
