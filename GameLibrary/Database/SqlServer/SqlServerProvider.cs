...
namespace GameLibrary.Database.SqlServer
{
    internal class SqlServerProvider : IDataProvider
    {
        private readonly string _connectionString;
        public SqlServerProvider(IDataConfiguration config)
        {
            _connectionString = config.DbConnectionString;
        }

        public void Execute(Action<IDataExecutor> action)
        {
            using var executor = new SqlServerExecutor(_connectionString);
            action(executor);
        }

        public T Execute<T>(Func<IDataExecutor, T> func)
        {
            using var executor = new SqlServerExecutor(_connectionString);
